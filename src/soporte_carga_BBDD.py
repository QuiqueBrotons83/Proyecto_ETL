...
def insertar_datos_en_BBDD(insert_query,data_to_insert):
    ...
    if isinstance(data_to_insert,list): # si data_to_insert es una lista
        cur.executemany(insert_query,data_to_insert) #ejecutar la acción de subida a la BBDD
    else: # si data_to_insert no es una lista
        cur.execute(insert_query,data_to_insert) #ejecutar la acción de subida a la BBDD
    conn.commit() #guardar la subida
...
def carga_completa_datos (ruta_importación_datos_limpios, ruta_entrada,ruta_guardar_eventos):
    ...
    """
    #CARGA LOS DATOS DE LA TABLA CIUDAD
    df_reservas = pd.read_pickle(ruta_importación_datos_limpios) #Importo la información del df de reservas ya limpio

    insert_query = "INSERT INTO ciudad(nombre_ciudad) VALUES(%s);" #crear la query de inserción
    data_to_insert = (df_reservas.ciudad[0],) #crear los datos a insertar
    print(f'Se ha cargado los datos en la tabla ciudad: Se han cargado:{len(data_to_insert)} registros')

    #CARGA LOS DATOS DE LA TABLA EVENTOS:
    sc.insertar_datos_en_BBDD(insert_query,data_to_insert) #Subir la información a la BBDD

    df_eventos = pd.DataFrame(sa.eventos_api(ruta_entrada,ruta_guardar_eventos)) #Extraer la información de la api y lo convierto en un df
    print(f'Se han extraido los eventos de la api y se ha guardado un fichero en la ruta {ruta_guardar_eventos}')

    query_extracción = "SELECT nombre_ciudad, id_ciudad FROM ciudad" #Crear la query para extraer los datos del id_ciudad de la BBDD
    ciudad_dict = sc.extraer_datos_de_BBDD(query_extracción) #Extraer el id de la ciudad de la BBDD
    

    #Crear los datos para insertarlos en la BBDD:
    data_to_insert=[]
    ...
    #Cargar LOS DATOS DE LA TABLA HOTELES:
    ...
    # CARGAR LOS DATOS DE LA TABLA RESERVAS:
    ...
    query_extracción="SELECT nombre_hotel, id_hotel FROM hoteles" #crear la query para extraer los datos del id_hotel de la BBDD
    hotel_dict=sc.extraer_datos_de_BBDD(query_extracción) #Extraer el id de la ciudad de la BBDD
